...
create_dir_cmd=sprintf("rm -rf %s;mkdir %s;chmod a+rx %s",out_dir,out_dir,out_dir)
...
bar_plot = function(top_n_apps,column_dis_b, column_dis_v){
  ...
  if (column_dis_v=="field_of_science" | column_dis_b=="field_of_science" ){
    ...
    if (column_dis_b=="field_of_science"){
      ...
      mfdt <- tmp[, lapply(.SD, sum), by=V2][c(1:8, 10:11)] # 9 is "REMOVE"
      ...
      dt<-dt[, 2:ncol(all), with=FALSE]
      mfdt <- dt[, lapply(.SD, sum), by=V2][c(1:8, 10:11)] # 9 is "REMOVE"
      m2 <- data.matrix(mfdt)
      if (column_dis_b=="field_of_science"){
        m2 = t(m2)
      }
      ...
      barplot(m2[, order(-colSums(m2))][,1:top_n_apps], cex.names=1, col=brewer.pal(nrow(m2), "Spectral"), las=2,
              main="Exectuable Usages by Fields of Science",
              ylab="Number of Executions", xlab="Executables")
      legend("topright", legend=mfdt$V2, fill=brewer.pal(nrow(m2), "Spectral"), cex = 1,box.col="white", ncol=1, y.intersp=0.8)
    }
    ...
    legend("topright", legend=rownames(data_sub_cores), fill=brewer.pal(nrow(data_sub_cores), "Spectral"), cex = 1,box.col="white", ncol=1, y.intersp=0.8)
